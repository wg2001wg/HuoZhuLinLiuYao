...
- **AI解析（联网大模型解卦，已取代原「系统解析」）**
  ...
  - **AI 解析结果持久化**：在结果页「保存到历史」时，若已有 AI 解读，会随排盘一并存入 Room；历史页回看该记录时直接显示已保存的解析，**无需重新联网**（卡片标注「已保存，离线可查看」）。历史页重新发起解析成功后也会自动回写该记录
  - **每个模型独立保存 API Key**：不同模型的 Key 分别存于本地，切换模型时**自动填充**对应模型的 Key
  - **自定义模型列表持久化**：在模型名输入框输入一个不存在的新模型名并保存后，会记入本地列表；下次设置页下拉可直接选择之前添加的自定义模型
  - 不阻塞本地排盘
...
2. 如需换用自己的模型：在应用首页点击 **「AI解析设置」**（也可在结果页 AI解析面板右上角齿轮进入），可配置三项：
   - **API Key**：留空使用内置默认 Key；如用智谱可在 [智谱开放平台](https://open.bigmodel.cn) 的「API Keys」页面创建。**每个模型的 Key 独立保存**，切换模型时会自动填充该模型已存的 Key（不同模型互不干扰）。
   - **模型名**：留空默认 `GLM-4.7-Flash`，可填写任意自定义模型（如 `deepseek-chat`、`gpt-4o-mini`、`qwen-plus` 等）。输入一个列表中没有的新模型名并保存后，会记入本地自定义模型列表，下次设置页下拉可直接选择。
...
4. 在**结果页**切到「AI解析」标签，或在**历史页**点击卡片上的解析图标，即会联网请求所配置的模型对当前卦象进行解读；结果页「保存到历史」会把 AI 解读一并存档，历史页回看时无需重新联网。
